...
- **每次新凭据使用批次，先说明用途、目标和范围，再主动向用户索取本次解密口令。** 不从聊天历史、旧笔记或模型总结复用口令/token来绕过询问。
- 旧批次“都上传”的授权不自动授权新仓库、新用途或新的凭据使用批次。用户拒绝或未提供口令则停止相应步骤。
- 不把口令、明文token、Authorization头、私钥、带密钥URL或完整聊天原文写进进度、Skill、日志、提交或交接包。只记“凭据密文在哪里、何时需要重新询问”。
- `handoff.py`拦截常见秘密格式和秘密字段，但它不是完整DLP。发布前仍需检查是否存在其他口令、个人隐私或业务机密。
- 私有库冷启动：若新环境缺少密文文件，用户须从其有权限的私有备份下载密文后提供，或自行提供进度包。**不能只凭一个公开Skill链接和口令，凭空下载藏在私有库里的密文。**
- 默认不把密文放入公开库或交接包；确需私有密文备份时需用户明确的目标。文件加密不能撤回此前聊天或工具输入记录。
...
2. 用户授权后同步到实际配置的私有进度仓库；公开仓库只放通用Skill、工具、模板。
